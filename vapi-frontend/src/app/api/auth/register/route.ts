 ...
 * Creates an account on the backend and immediately establishes the session
 * cookie, so a successful sign-up lands directly in the dashboard.
 */
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    upstream = await fetch(`${BACKEND_API_URL}/api/auth/register`, {
      method: 'POST',
  ...
  if (!upstream.ok || !data?.token) {
    return NextResponse.json(
      data ?? { error: { code: 'UPSTREAM_ERROR', message: 'Registration failed' } },
